...
            .from('transactions')
            .select('*, google_id, game, base_amount') // <--- MODIFICACIÓN 1: Incluimos base_amount
...
        const { google_id, game, "finalPrice": finalPrice, currency, base_amount } = transactionData; // <--- MODIFICACIÓN 2: Extraemos base_amount
        
...
        console.log(`TRAZA 11: Transacción encontrada. Game: ${game}, Google ID: ${google_id}`);
        console.log(`TRAZA 11.0: Monto base (sin comisión): ${base_amount} USD. Monto a inyectar: ${amountToInject} USD.`); // <--- Nueva traza
        
...
                 injectionMessage = `\n\n❌ **ERROR DE INYECCIÓN DE SALDO:** Datos incompletos (Google ID: ${google_id}, Monto: ${amountToInject}). **¡REVISIÓN MANUAL REQUERIDA!**`; // Ajustado el mensaje de error para mostrar amountToInject
...
                             p_user_id: google_id, 
                             p_monto: amountToInject.toFixed(2) // Usamos amountToInject, que es base_amount
                         }); 
                         
