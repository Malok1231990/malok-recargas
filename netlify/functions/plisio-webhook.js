// netlify/functions/plisio-webhook.js (CORREGIDO: Hash Constante)
...
exports.handler = async (event, context) => {
    ...
    // 💡 SOLUCIÓN AL HASH CONSTANTE: Reconstruir la cadena JSON en orden alfabético
    // Esto asegura que la función genere el mismo hash que Plisio y que calculamos con Node.js.
    const payloadToHash = { ...body };
    delete payloadToHash.verify_hash; // Eliminar el hash recibido antes de stringificar
    ...
    for (let i = 0; i < sortedKeys.length; i++) {
        ...
        const value = payloadToHash[key];
        
        // Plisio requiere que todos los valores sean strings, incluyendo números
        const valueString = typeof value === 'number' ? String(value) : value;
        
        stringToHash += `"${key}":"${valueString}"`;
    ...
    // 🚨 TRAZA 5.3: Mostrar el string que se está firmando (útil para debugging)
    // console.log("TRAZA 5.3: String to Hash:", stringToHash);
    ...
    try {
        ...
        // MODIFICACIÓN 3: Determinar el monto a inyectar
        let amountToInject; 
